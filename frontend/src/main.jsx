...
// v26 Admin PRO multilingual user interface (ES / FA / AR)
const ADMIN_UI_LANGUAGE_KEY = 'cdkt_admin_ui_language';
...
function applyAdminUiLanguage(language) {
  ...
  root.querySelectorAll('button, h1, h2, h3, h4, p, span, small, th, td, label, option, a, div').forEach(node => {
    ...
    const original = adminUiOriginalText(node);
    const trimmed = original.trim();
    if (!trimmed) return;
    const translated = language === 'es' ? original : (translation[trimmed] || original);
  ...
  root.querySelectorAll('input[placeholder], textarea[placeholder]').forEach(node => {
    if (!node.dataset.cdktOriginalPlaceholder) node.dataset.cdktOriginalPlaceholder = node.getAttribute('placeholder') || '';
    const original = node.dataset.cdktOriginalPlaceholder;
    const translated = language === 'es' ? original : (translation[original] || original);
    node.setAttribute('placeholder', translated);
  });
  document.documentElement.lang = language === 'fa' ? 'fa' : language === 'ar' ? 'ar' : 'es';
...
function AdminLanguageSwitch() {
  ...
  useEffect(() => {
    const apply = () => applyAdminUiLanguage(localStorage.getItem(ADMIN_UI_LANGUAGE_KEY) || 'es');
    apply();
...
function Header({ title = 'Casa de Kebab Turco', subtitle = RESTAURANT_ADDRESS, children }) {
  ...
  return <header className="topbar">
    ...
    <nav>
      ...
      {getAdminToken() && isAdminRoute() && <AdminLanguageSwitch />}
    </nav>
  </header>;
}
...
function RealInventoryPanel({ expenseCategories = [] }) {
  ...
  const tr = {
    es: { title:'Compras e Inventario Real', sub:'Crea materias primas, registra compras con IVA, mermas y corrige el stock contado.', refresh:'Actualizar', addIngredient:'Añadir materia prima', purchase:'Registrar compra', waste:'Registrar merma', adjust:'Corregir stock', stock:'Stock real', suggested:'Lista de compra sugerida (7 días)', movements:'Movimientos recientes', ingredient:'Ingrediente', ingredientName:'Nombre de la materia prima', unit:'Unidad', unitCost:'Coste por unidad sin IVA (€)', initialStock:'Stock inicial', minimumStock:'Stock mínimo / alerta', quantity:'Cantidad', counted:'Stock real contado', current:'Stock actual del sistema', difference:'Diferencia que se registrará', subtotal:'Importe sin IVA', ivaRate:'IVA (%)', ivaAmount:'Importe IVA', total:'Total a pagar (con IVA)', supplier:'Proveedor', invoice:'Factura', notes:'Motivo / notas', save:'Guardar compra', create:'Crear materia prima', days:'Días', use:'Consumo/día', noData:'No hay ingredientes configurados.', purchaseDone:'Compra, IVA y gasto registrados correctamente.', wasteDone:'Merma registrada.', adjustmentDone:'Stock corregido y movimiento de ajuste guardado.', ingredientDone:'Materia prima creada. Ya puedes registrar su compra.', type:'Tipo', delta:'Cambio', stockNow:'Stock actual', danger:'Urgente', low:'Bajo', ok:'Correcto', needName:'Escribe el nombre de la materia prima.', selectIngredient:'Selecciona una materia prima.', noChange:'El stock contado es igual al stock actual; no hay nada que corregir.', adjustmentHelp:'Usa esta opción después de contar físicamente el almacén. No registra un gasto ni una compra.', ivaHint:'El coste unitario es sin IVA. El sistema calcula el total automáticamente.' },
    fa: { title:'خرید و انبار واقعی', sub:'ماده اولیه بسازید، خرید را با IVA ثبت کنید، ضایعات را وارد کنید و موجودی شمارش‌شده را اصلاح کنید.', refresh:'به‌روزرسانی', addIngredient:'افزودن ماده اولیه', purchase:'ثبت خرید', waste:'ثبت ضایعات', adjust:'اصلاح موجودی', stock:'موجودی واقعی', suggested:'لیست خرید پیشنهادی (۷ روز)', movements:'گردش‌های اخیر', ingredient:'ماده اولیه', ingredientName:'نام ماده اولیه', unit:'واحد', unitCost:'هزینه هر واحد بدون IVA (€)', initialStock:'موجودی اولیه', minimumStock:'حداقل موجودی / هشدار', quantity:'تعداد / مقدار خرید', counted:'موجودی واقعی شمارش‌شده', current:'موجودی فعلی سیستم', difference:'اختلافی که ثبت می‌شود', subtotal:'مبلغ بدون IVA', ivaRate:'درصد IVA', ivaAmount:'مبلغ IVA', total:'مبلغ نهایی با IVA', supplier:'تأمین‌کننده', invoice:'شماره فاکتور', notes:'علت / توضیحات', save:'ثبت خرید', create:'ساخت ماده اولیه', days:'روز', use:'مصرف/روز', noData:'ماده اولیه‌ای تعریف نشده است.', purchaseDone:'خرید، IVA و هزینه با موفقیت ثبت شد.', wasteDone:'ضایعات ثبت شد.', adjustmentDone:'موجودی اصلاح و گردش اصلاح ثبت شد.', ingredientDone:'ماده اولیه ساخته شد؛ اکنون می‌توانید خرید آن را ثبت کنید.', type:'نوع', delta:'تغییر', stockNow:'موجودی فعلی', danger:'فوری', low:'کم', ok:'مناسب', needName:'نام ماده اولیه را وارد کنید.', selectIngredient:'یک ماده اولیه انتخاب کنید.', noChange:'موجودی شمارش‌شده با موجودی فعلی برابر است و نیازی به اصلاح نیست.', adjustmentHelp:'پس از شمارش واقعی انبار از این گزینه استفاده کنید. این عملیات خرید یا هزینه مالی ثبت نمی‌کند.', ivaHint:'هزینه واحد بدون IVA است؛ سیستم مبلغ نهایی را خودکار محاسبه می‌کند.' },
    ar: { title:'المشتريات والمخزون الفعلي', sub:'أنشئ المواد الأولية وسجل الشراء مع IVA والهدر وصحح المخزون المعدود.', refresh:'تحديث', addIngredient:'إضافة مادة أولية', purchase:'تسجيل شراء', waste:'تسجيل هدر', adjust:'تصحيح المخزون', stock:'المخزون الفعلي', suggested:'قائمة شراء مقترحة (7 أيام)', movements:'آخر الحركات', ingredient:'المادة', ingredientName:'اسم المادة الأولية', unit:'الوحدة', unitCost:'تكلفة الوحدة بدون IVA (€)', initialStock:'المخزون الافتتاحي', minimumStock:'الحد الأدنى / تنبيه', quantity:'الكمية المشتراة', counted:'المخزون الفعلي المعدود', current:'مخزون النظام الحالي', difference:'الفرق الذي سيتم تسجيله', subtotal:'المبلغ بدون IVA', ivaRate:'نسبة IVA', ivaAmount:'مبلغ IVA', total:'المبلغ الإجمالي مع IVA', supplier:'المورد', invoice:'رقم الفاتورة', notes:'السبب / ملاحظات', save:'حفظ الشراء', create:'إنشاء المادة الأولية', days:'الأيام', use:'الاستهلاك/يوم', noData:'لا توجد مواد أولية مضافة.', purchaseDone:'تم تسجيل الشراء وIVA والمصروف بنجاح.', wasteDone:'تم تسجيل الهدر.', adjustmentDone:'تم تصحيح المخزون وحفظ حركة التصحيح.', ingredientDone:'تم إنشاء المادة الأولية ويمكنك الآن تسجيل شرائها.', type:'النوع', delta:'التغيير', stockNow:'المخزون الحالي', danger:'عاجل', low:'منخفض', ok:'جيد', needName:'اكتب اسم المادة الأولية.', selectIngredient:'اختر مادة أولية.', noChange:'المخزون المعدود يساوي المخزون الحالي ولا يوجد شيء لتصحيحه.', adjustmentHelp:'استخدم هذا الخيار بعد العد الفعلي للمخزون. لا يسجل شراءً أو مصروفاً مالياً.', ivaHint:'تكلفة الوحدة بدون IVA، ويحسب النظام الإجمالي تلقائياً.' },
  }[language];

  const load = async () => {
    try { setLoading(true); const res = await axios.get(`${API_BASE}/admin/inventory/overview/?days=14`); setData(res.data || {}); }
    catch (err) { window.alert(err?.response?.data?.detail || 'No se pudo cargar el inventario.'); }
  ...
  const ivaPercent = Math.max(0, Math.min(100, Number(form.iva_percent || 0)));
  const subtotal = quantity * unitCost;
  const ivaAmount = subtotal * ivaPercent / 100;
  const totalAmount = subtotal + ivaAmount;
  const adjustmentDelta = selected && form.counted_quantity !== '' ? Number(form.counted_quantity) - Number(selected.stock_quantity || 0) : null;
  const chooseIngredient = value => {
    const item = (data.ingredients || []).find(x => String(x.id) === String(value));
    setForm(current => ({ ...current, ingredient_id:value, counted_quantity:'', unit_cost:item ? String(item.unit_cost ?? '') : current.unit_cost }));
  };

  const submitMovement = async e => {
    e.preventDefault();
    if (!form.ingredient_id) { window.alert(tr.selectIngredient); return; }
    let endpoint = 'waste';
    let payload = { ingredient_id:form.ingredient_id, quantity:form.quantity, notes:form.notes };
    if (mode === 'purchase') {
      endpoint = 'purchase';
      payload = { ...form, unit_cost:unitCost.toFixed(4), iva_percent:ivaPercent.toFixed(2), subtotal_amount:subtotal.toFixed(2), iva_amount:ivaAmount.toFixed(2), total_amount:totalAmount.toFixed(2) };
    } else if (mode === 'adjust') {
      endpoint = 'adjustment';
      if (adjustmentDelta === null || Number.isNaN(adjustmentDelta) || Math.abs(adjustmentDelta) < 0.000001) { window.alert(tr.noChange); return; }
      payload = { ingredient_id:form.ingredient_id, quantity_delta:adjustmentDelta.toFixed(3), notes:form.notes };
    }
    try {
      await axios.post(`${API_BASE}/admin/inventory/${endpoint}/`, payload);
      window.alert(mode === 'purchase' ? tr.purchaseDone : mode === 'waste' ? tr.wasteDone : tr.adjustmentDone);
      setForm(current => ({...current, quantity:'', counted_quantity:'', invoice_number:'', notes:''}));
      await load();
  ...
  const createIngredient = async e => {
    ...
    try {
      const res = await axios.post(`${API_BASE}/admin/profitability/ingredients/`, {
        ...ingredientForm, unit_cost:ingredientForm.unit_cost || 0, stock_quantity:ingredientForm.stock_quantity || 0,
        reorder_level:ingredientForm.reorder_level || 0, is_active:true,
      });
      window.alert(tr.ingredientDone);
      setForm(current => ({ ...current, ingredient_id:String(res.data?.id || ''), unit_cost:String(res.data?.unit_cost ?? ingredientForm.unit_cost ?? '') }));
      setIngredientForm({ name:'', unit:'kg', unit_cost:'', stock_quantity:'0', reorder_level:'', supplier_name:'' });
      setMode('purchase'); await load();
  ...
  return <section className={`real-inventory-page lang-${language}`} dir={language === 'es' ? 'ltr' : 'rtl'}>
    ...
    <section className="real-inventory-grid">
      <article className="admin-card inventory-action-card">
        <div className="real-mode-switch"><button type="button" className={mode==='add'?'active':''} onClick={()=>setMode('add')}>{tr.addIngredient}</button><button type="button" className={mode==='purchase'?'active':''} onClick={()=>setMode('purchase')}>{tr.purchase}</button><button type="button" className={mode==='waste'?'active':''} onClick={()=>setMode('waste')}>{tr.waste}</button><button type="button" className={mode==='adjust'?'active':''} onClick={()=>setMode('adjust')}>{tr.adjust}</button></div>
        {mode === 'add' ? <form className="smart-recurring-form" onSubmit={createIngredient}>
          ...
          <select value={ingredientForm.unit} onChange={e=>setIngredientForm({...ingredientForm,unit:e.target.value})}><option value="kg">kg</option><option value="g">g</option><option value="l">l</option><option value="ml">ml</option><option value="unit">unidad</option><option value="pack">pack</option></select>
          ...
          <input placeholder={tr.supplier} value={ingredientForm.supplier_name} onChange={e=>setIngredientForm({...ingredientForm,supplier_name:e.target.value})}/><button className="pay">{tr.create}</button>
        </form> : <form className="smart-recurring-form" onSubmit={submitMovement}>
          <select required value={form.ingredient_id} onChange={e=>chooseIngredient(e.target.value)}><option value="">{tr.ingredient}</option>{(data.ingredients||[]).map(i=><option key={i.id} value={i.id}>{i.name} · {i.stock_quantity} {i.unit}</option>)}</select>
          {mode==='adjust' ? <div className="inventory-adjust-box"><p>{tr.adjustmentHelp}</p>{selected && <div className="inventory-current-stock"><b>{tr.current}:</b> {selected.stock_quantity} {selected.unit}</div>}<input required type="number" min="0" step="0.001" placeholder={`${tr.counted}${selected ? ` (${selected.unit})` : ''}`} value={form.counted_quantity} onChange={e=>setForm({...form,counted_quantity:e.target.value})}/>{adjustmentDelta !== null && !Number.isNaN(adjustmentDelta) && <div className={`inventory-adjust-delta ${adjustmentDelta > 0 ? 'positive' : adjustmentDelta < 0 ? 'negative' : ''}`}><b>{tr.difference}:</b> {adjustmentDelta > 0 ? '+' : ''}{adjustmentDelta.toFixed(3)} {selected?.unit || ''}</div>}</div> : <>
            ...
            {mode==='purchase' && <><input required type="number" min="0" step="0.0001" placeholder={tr.unitCost} value={form.unit_cost} onChange={e=>setForm({...form,unit_cost:e.target.value})}/><input type="number" min="0" max="100" step="0.01" placeholder={tr.ivaRate} value={form.iva_percent} onChange={e=>setForm({...form,iva_percent:e.target.value})}/><div className="purchase-iva-summary"><p>{tr.ivaHint}</p><div><span>{tr.subtotal}</span><b>{money(subtotal)}</b></div><div><span>{tr.ivaAmount} ({ivaPercent.toFixed(2)}%)</span><b>{money(ivaAmount)}</b></div><div className="purchase-iva-total"><span>{tr.total}</span><b>{money(totalAmount)}</b></div></div><input placeholder={tr.supplier} value={form.supplier_name} onChange={e=>setForm({...form,supplier_name:e.target.value})}/><input placeholder={tr.invoice} value={form.invoice_number} onChange={e=>setForm({...form,invoice_number:e.target.value})}/><select value={form.paid_by} onChange={e=>setForm({...form,paid_by:e.target.value})}><option value="bbva">BBVA</option><option value="saeid">Saeid</option><option value="ahmed">Ahmed</option></select></>}
          </>}
          <textarea placeholder={tr.notes} value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})}/><button className="pay">{mode==='adjust' ? tr.adjust : tr.save}</button>
        </form>}
      </article>
